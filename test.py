...
class TestCalculator(unittest.TestCase):
	...
	def test_mul1(self):
		...
		solution = self.app.get('/mul?A=22&B=2')
		self.assertEqual(b'44.0', solution.data)

	def test_mul2(self):

		#case 2, A is rational number and B is rational number p/q form
		solution = self.app.get('/mul?A=3/2&B=1/4')
		self.assertEqual(b'0.375', solution.data)

	def test_mul3(self):

		#case 3, A is a float and B is a float
		solution = self.app.get('/mul?A=12.22&B=1.22')
		self.assertEqual(b'14.9084', solution.data)

	def test_mul4(self):

		#case 4, when A is integer and B is float
		solution = self.app.get('/mul?A=1000&B=9.99')
		self.assertEqual(b'9990.0', solution.data)

	def test_mul5(self):

		#case 5, when A is fraction p/q and B is an integer
		solution = self.app.get('/mul?A=11&B=99')
		self.assertEqual(b'1089.0', solution.data)

	def test_mul6(self):

		#case 6, when A is an integer and B is a fraction p/q
		solution = self.app.get('/mul?A=23&B=9/13')
		self.assertEqual(b'15.923076923076923', solution.data)

	def test_mul7(self):

		#case 7, when A input is an alphabet(non integer) and B is integer
		solution = self.app.get('/mul?A=char&B=22')
		self.assertEqual(b'0.0', solution.data)#non integer type considered as not valid , in this case which is zero

	def test_mul8(self):

		#case 8, when A input is an integer and B input is an alphabet
		solution = self.app.get('/mul?A=2&B=char')
	...
	def test_mul9(self):
		...
		#case 9, when A input is of the form p/q where q=0 and B input be any number
	...
	def test_mul10(self):
		...
		#case 10, when A input is any number and B=p/q form where q=0

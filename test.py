...
class TestCalculator(unittest.TestCase):
	...
	def test_sub1(self):
		...
		solution = self.app.get('/sub?A=4&B=2')
	...
	def test_sub3(self):
		...
		solution = self.app.get('/sub?A=10.2&B=1.2')
		self.assertEqual(b'9.0', solution.data)

	def test_sub4(self):

		#case 4, when A is float and B is integer
		solution = self.app.get('/sub?A=2.2&B=2')
		self.assertEqual(b'0.2', solution.data)

	def test_sub5(self):

		#case 5, when A is integer and B is float
		solution = self.app.get('/sub?A=12&B=1.88')
		self.assertEqual(b'10.12', solution.data)

	def test_sub6(self):

		#case 6, when A is fraction p/q and B is an integer
		solution = self.app.get('/sub?A=11/2&B=1')
		self.assertEqual(b'4.5', solution.data)
	...
	def test_sub8(self):
		...
		solution = self.app.get('/sub?A=char&B=1')
		self.assertEqual(b'-1.0', solution.data)#non integer type considered as not valid , in this case which is zero

	def test_sub9(self):

		#case 9, when A input is an integer and B input is an alphabet
		solution = self.app.get('/sub?A=12&B=char')
	...
	def test_sub10(self):
		...
		solution = self.app.get('sub?A=1/0&B=12')
	...
	def test_sub11(self):
		...
		solution = self.app.get('sub?A=22&B=2/0')
		self.assertEqual(b"None", solution.data)
